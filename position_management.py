...
def create_position_tables():
    ...
    try:
        ...
        CREATE TABLE IF NOT EXISTS asset_snapshot (
            ...
            market VARCHAR(20) NOT NULL COMMENT '市场代码（AStockMarket/HStockMarket等）',
            ...
            UNIQUE KEY uk_date_market (snapshot_date, market),
        ...
        CREATE TABLE IF NOT EXISTS position_snapshot (
            id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
            snapshot_date DATE NOT NULL COMMENT '快照日期',
            market VARCHAR(20) NOT NULL COMMENT '市场代码',
            ...
            UNIQUE KEY uk_date_market_stock (snapshot_date, market, stock_code),
            INDEX idx_snapshot_date (snapshot_date),
            INDEX idx_market (market),
        ...
        CREATE TABLE IF NOT EXISTS current_position (
            ...
            UNIQUE KEY uk_date_market_stock (calculation_date, market, stock_code),
...
def get_latest_snapshot_file(market='AStockMarket'):
    """
    获取指定市场的最新资产快照文件

    参数:
        market: 市场代码（如'AStockMarket', 'HStockMarket'）

    返回:
        tuple: (文件路径, 快照日期, 市场代码)
    """
    market_dir = os.path.join(SNAPSHOT_DIR, market)
    if not os.path.exists(market_dir):
        print(f"[ERROR] 市场目录不存在: {market_dir}")
        return None, None, market

    # 查找所有快照文件
    # 支持两种格式：
    # 1. A股: YYYYMMDD 资金股份查询.txt
    # 2. H股: YYYYMMDD .txt (文件名只是日期+空格+扩展名)
    snapshot_files = []

    # A股格式
    pattern_a = re.compile(r'^(\d{8})\s+资金股份查询\.(txt|xls)$')
    # H股格式（文件名只有日期+空格）
    pattern_h = re.compile(r'^(\d{8})\s+\.(txt|xls)$')

    for filename in os.listdir(market_dir):
        match = pattern_a.match(filename) or pattern_h.match(filename)
        if match:
    ...
    if not snapshot_files:
        print(f"[ERROR] 未找到有效的快照文件（市场: {MARKETS.get(market, market)}）")
        print(f"  支持格式: YYYYMMDD 资金股份查询.txt 或 YYYYMMDD .txt")
        return None, None, market
    ...
    print(f"[OK] 找到最新快照文件: {os.path.basename(latest_file)} (市场: {MARKETS.get(market, market)}, 日期: {latest_date})")
    return latest_file, latest_date, market
...
def read_snapshot_file(filepath):
    ...
    try:
        ...
        print(f"[OK] 文件读取成功，共 {len(lines)} 行")

        # 解析资产信息（第一行）
        # A股格式：可用:2971.44  可取:6097269.58  获取:2971.44  余颖:6094299.69  证券市值:3350251.14  总资产:9552735.94  盈亏:-20201.52
        # H股格式：无资产信息行，直接是持仓数据
        asset_info = {
            'snapshot_date': None,
            'total_assets': None,
        ...
        for line in lines:
            ...
            try:
                ...
                if parts[0][0].isdigit():
                    # A股格式：证券代码 证券名称 股东代码 可用数量 成本价 当前价 市值 盈亏 盈亏比例(%)
                    stock_code = parts[0].strip()
                    numeric_start = 1  # 从第二列开始找证券名称
                else:
                    # H股格式：证券市场 证券代码 证券名称 股份数量 预留股份数 成本价(港币) 当前价(港币) 市场总值(港币) 实现盈亏(港币) 盈亏比例(%)
                    if len(parts) >= 2 and parts[1][0].isdigit():
                        stock_code = parts[1].strip()
                        numeric_start = 2  # 从第三列开始找证券名称
                ...
                # 找到数值字段的起始位置
                data_start = numeric_start + len(name_parts)

                if data_start + 8 > len(parts):
                    continue

                # 根据格式解析字段
                if numeric_start == 1:
                    # A股格式
                    position_count = float(parts[data_start]) if data_start < len(parts) else 0
                    available_count = float(parts[data_start + 1]) if data_start + 1 < len(parts) else 0
                    cost_price = float(parts[data_start + 2]) if data_start + 2 < len(parts) else 0
                    current_price = float(parts[data_start + 3]) if data_start + 3 < len(parts) else 0
                    market_value = float(parts[data_start + 4]) if data_start + 4 < len(parts) else 0
                    profit_loss = float(parts[data_start + 5]) if data_start + 5 < len(parts) else 0
                    profit_loss_ratio = float(parts[data_start + 6]) if data_start + 6 < len(parts) else 0
                else:
                    # H股格式：股份数量 预留股份数 成本价 当前价 市场总值 实现盈亏 盈亏比例
                    position_count = float(parts[data_start]) if data_start < len(parts) else 0
                    available_count = float(parts[data_start + 1]) if data_start + 1 < len(parts) else 0
                    cost_price = float(parts[data_start + 2]) if data_start + 2 < len(parts) else 0
                    current_price = float(parts[data_start + 3]) if data_start + 3 < len(parts) else 0
        ...
        print(f"[OK] 解析到 {len(positions)} 个持仓")
        ...
        print(f"[ERROR] 读取快照文件失败: {e}")
        import traceback
        traceback.print_exc()
        return None, []


def get_operation_log_files(after_date=None, market='AStockMarket'):
    """
    获取指定市场的操作对账单文件列表
    ...
    返回:
        list: [(文件路径, 对账单日期), ...]
    """
    market_dir = os.path.join(OPERATION_LOG_DIR, market)
    if not os.path.exists(market_dir):
        print(f"[ERROR] 市场对账单目录不存在: {market_dir}")
        return []

    # 查找所有对账单文件
    operation_files = []
    # A股格式: YYYYMMDD 对帐单查询.txt
    pattern_a = re.compile(r'^(\d{8})\s+对帐单查询\.(txt|xls)$')
    # H股格式: YYYYMMDD .txt
    pattern_h = re.compile(r'^(\d{8})\s+\.(txt|xls)$')
    ...
    print(f"[OK] 找到 {len(operation_files)} 个对账单文件 (市场: {MARKETS.get(market, market)})")
...
def read_operation_log_file(filepath):
    ...
    try:
        ...
        print(f"[OK] 解析到 {len(operations)} 条操作记录")
...
def save_snapshot_to_db(snapshot_date, market, asset_info, positions):
    """
    将快照数据保存到数据库
    ...
    try:
        ...
        INSERT INTO asset_snapshot
            (snapshot_date, market, total_assets, cash_balance, market_value,
        ...
        INSERT INTO position_snapshot
            (snapshot_date, market, stock_code, stock_name, position_count, available_count,
        ...
        print(f"[OK] 快照数据已保存到数据库 (日期: {snapshot_date}, 市场: {MARKETS.get(market, market)})")
        return True

    except Exception as e:
        print(f"[ERROR] 保存快照数据失败: {e}")
        import traceback
        traceback.print_exc()
        return False


def save_operations_to_db(operations, market='AStockMarket'):
    """
    将操作记录保存到数据库

    参数:
        operations: 操作记录列表
        market: 市场代码 (默认: 'AStockMarket')
    """
    try:
        conn = pymysql.connect(**DB_CONFIG)
        cursor = conn.cursor()
        ...
        INSERT INTO stock_operation_log
            (operation_date, trade_time, stock_code, stock_name, operation_type,
             ...
             stamp_duty, other_fees, net_amount, remark, market)
        ...
        for op in operations:
            try:
                ...
                print(f"  [WARNING] 插入操作记录失败: {op.get('stock_code')} - {e}")

        conn.commit()
        cursor.close()
        conn.close()

        print(f"[OK] 已保存 {count} 条操作记录到数据库 (市场: {MARKETS.get(market, market)})")
        return count

    except Exception as e:
        print(f"[ERROR] 保存操作记录失败: {e}")
        import traceback
        traceback.print_exc()
        return 0


def get_latest_snapshot_from_db(market='AStockMarket'):
    """
    从数据库获取最新的资产快照和持仓
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE snapshot_date = %s AND market = %s
        ...
        print(f"[OK] 从数据库读取快照数据 (日期: {snapshot_date}, 市场: {MARKETS.get(market, market)})")
        return snapshot_date, asset_info, positions

    except Exception as e:
        print(f"[ERROR] 从数据库读取快照失败: {e}")
        return None, None, []


def get_operations_after_date(after_date, market='AStockMarket'):
    """
    从数据库获取指定日期之后的操作记录
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE operation_date > %s AND market = %s
        ...
        print(f"[OK] 从数据库读取 {len(operations)} 条操作记录 (日期: {after_date} 之后, 市场: {MARKETS.get(market, market)})")
        return operations

    except Exception as e:
        print(f"[ERROR] 从数据库读取操作记录失败: {e}")
        return []
...
def calculate_current_position(snapshot_date, snapshot_positions, operations):
    ...
    print(f"[OK] 从快照初始化 {len(position_map)} 个持仓")
    ...
    for op in operations:
        ...
        if '买入' in op_type or op_count > 0:
            ...
            new_count = old_count + decimal.Decimal(abs(op_count))

            if new_count > 0:
                position['avg_cost_price'] = decimal.Decimal(decimal.Decimal(old_cost_value) + decimal.Decimal(buy_amount)) / new_count
                position['current_count'] = new_count
                position['available_count'] += decimal.Decimal(abs(op_count))
                position['cost_value'] = old_cost_value + decimal.Decimal(buy_amount)
            else:
        ...
        if position['current_count'] > 0:
            position['market_value'] = position['current_count'] * decimal.Decimal(position['last_price'])
            position['profit_loss'] = decimal.Decimal(position['market_value']) - decimal.Decimal(position['cost_value'])
            if position['cost_value'] > 0:
                position['profit_loss_ratio'] = position['profit_loss'] / decimal.Decimal(position['cost_value'])
...
def save_current_position_to_db(calculation_date, market, current_positions, summary):
    """
    将计算结果保存到数据库
    ...
    try:
        ...
        # 先删除该日期和市场的旧数据
        cursor.execute("DELETE FROM current_position WHERE calculation_date = %s AND market = %s",
                      (calculation_date, market))

        # 插入新数据
        insert_sql = """
        INSERT INTO current_position
            (calculation_date, market, stock_code, stock_name, current_count, available_count,
        ...
        print(f"[OK] 当前持仓已保存到数据库 (日期: {calculation_date}, 市场: {MARKETS.get(market, market)})")
        return True

    except Exception as e:
        print(f"[ERROR] 保存当前持仓失败: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def print_current_positions(current_positions, summary):
    ...
    print(f"  买入次数: {summary['buy_count']}, 买入金额: CNY{summary['total_buy_amount']:,.2f}")
    print(f"  卖出次数: {summary['sell_count']}, 卖出金额: CNY{summary['total_sell_amount']:,.2f}")
    print(f"  总市值: CNY{summary['total_market_value']:,.2f}")
    print(f"  总成本: CNY{summary['total_cost_value']:,.2f}")
    print(f"  总盈亏: CNY{summary['total_profit_loss']:,.2f} ({summary['total_profit_loss_ratio']*100:.2f}%)")
    ...
    for i, pos in enumerate(current_positions, 1):
        ...
        print(f"{i:<4} {pos['stock_code']:<12} {pos['stock_name']:<10} "
              ...
              f"CNY{pos['market_value']:>11,.2f} CNY{pos['cost_value']:>11,.2f} "
              f"CNY{pos['profit_loss']:>11,.2f} {profit_loss_ratio_pct:>7.2f}%")
...
def main(market='AStockMarket'):
    ...
    """
    print(f"\n{'#'*80}")
    print(f"# 仓位管理系统 - {MARKETS.get(market, market)}")
    ...
    print("\n【步骤2】读取最新资产快照...")
    latest_snapshot_file, file_snapshot_date, _ = get_latest_snapshot_file(market=market)
    db_snapshot_date, db_asset_info, db_snapshot_positions = get_latest_snapshot_from_db(market=market)
    ...
    if latest_snapshot_file and db_snapshot_date:
        ...
        if file_snapshot_date > db_snapshot_date:
            print(f"  [OK] 使用文件快照（更新）")
            use_file_snapshot = True
        elif file_snapshot_date < db_snapshot_date:
            print(f"  [OK] 使用数据库快照（更新）")
            use_db_snapshot = True
        else:
            # 日期相同，优先使用数据库（因为可能已包含更多信息）
            print(f"  [OK] 日期相同，使用数据库快照")
            use_db_snapshot = True

    elif latest_snapshot_file:
        # 只有文件
        print(f"[DATE] 仅找到文件快照: {file_snapshot_date}")
        use_file_snapshot = True

    elif db_snapshot_date:
        # 只有数据库
        print(f"[DATE] 仅找到数据库快照: {db_snapshot_date}")
        use_db_snapshot = True
    else:
        # 都没有
        print("[ERROR] 未找到任何快照数据，程序退出")
        return
    ...
    if use_file_snapshot:
        ...
        if snapshot_positions:
            save_snapshot_to_db(snapshot_date, market, asset_info, snapshot_positions)
    ...
    # 3. 读取操作对账单（快照日期之后的）
    print("\n【步骤3】读取操作对账单...")
    operation_files = get_operation_log_files(after_date=snapshot_date, market=market)
    ...
    for op_file, _ in operation_files:
        ...
        if operations:
            ...
            # 保存到数据库
            save_operations_to_db(operations, market=market)

    # 如果没有新文件，从数据库读取
    if not operation_files:
        print("未找到新的对账单文件，从数据库读取操作记录...")
        all_operations = get_operations_after_date(snapshot_date, market=market)
    ...
    print("\n【步骤5】保存计算结果...")
    calculation_date = datetime.now().date()
    save_current_position_to_db(calculation_date, market, current_positions, summary)
    ...
    print(f"{'#'*80}")
    print(f"# 仓位管理完成 - {MARKETS.get(market, market)}")
    print(f"{'#'*80}\n")
...
if __name__ == '__main__':
    # 可以指定市场参数，例如：main(market='HStockMarket')
    # 或者处理所有市场
    market = 'AStockMarket'  # 默认处理A股
    main(market=market)
    market = 'HStockMarket'  # 默认处理A股
    main(market=market)
